...
import SonicProfile from "@/components/SonicProfileDashboard";
import { formatTotalDuration, getReleaseYear } from "@/utils/formatTime";
import { getAlbumData } from "@/services/album";
import { Album } from "@repo/types/src/artistsAlbumsSongs/album";
import ArtistsText from "@/components/ArtistsText";

export default async function AlbumPage({ params }) {
  const resolvedParams = await params;
  const albumData: Album | null = await getAlbumData(resolvedParams.albumId);
            ...
            <div className="flex-1 space-y-5">
              ...
              <div className="flex items-center gap-4 text-gray-400 font-medium">
                <ArtistsText 
                    ...
                    mainClass="cursor-pointer transition-colors text-lg text-white font-bold hover:text-violet-500"
                    links={true}
                    features={false}
                 />
                ...
                <span className="text-sm">
                  ...
                  {albumData.tracks.length} canciones •{" "}
                  {formatTotalDuration(albumData.duration)}
                </span>
              </div>
              <RatingAndQuickActions data={albumData} type="album" />
            </div>
          </div>
        </div>
      </header>
