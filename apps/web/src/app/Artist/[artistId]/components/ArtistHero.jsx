...
const ArtistHero = ({ artistData }) => {    
    ...
    // Verificamos si la imagen es el placeholder por defecto
    const isPlaceholder = artistData.portrait === null || artistData.portrait === undefined || artistData.portrait === "null";
        ...
        <header className="relative min-h-[68vh] flex items-end overflow-hidden border-b border-white/5">
            ...
            <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-12 pt-14 md:pt-20">
                <div className="flex flex-col md:flex-row gap-10 md:gap-12 items-center md:items-end text-center md:text-left">
                    ...
                    {/* Contenedor de la Imagen / Iniciales */}
                    <div className="relative shrink-0 flex items-center justify-center rounded-full border-4 border-white/5 bg-[linear-gradient(145deg,rgba(255,255,255,0.05),rgba(255,255,255,0.015))] shadow-[0_0_80px_rgba(139,92,246,0.18)] overflow-hidden w-64 h-64 md:w-80 md:h-80">
                        ...
                        {!isPlaceholder ? (
                            ...
                            <span className="text-7xl md:text-9xl font-black text-white/70 select-none tracking-tighter">
                                {getInitials(artistData.name)}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="flex-1 space-y-6 pt-12">
                        <div className="space-y-4">
                            <h1 className="text-4xl mb-12 md:text-6xl lg:text-7xl font-black uppercase tracking-tighter leading-none"> 
                                {artistData.name} 
                            </h1>
                        ...
                        <RatingAndQuickActions 
                            ...
                            ratingHref={`/Reviews/Artist/${artistData.name}`} 
                            type='artist' 
                            links={artistData.externalLinks} 
                        />
                    </div>
                </div>
            </div>
