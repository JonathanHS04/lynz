...
import { Song, Album } from '@repo/types';

const Hero = ({ featuredAlbum, featuredSong }: { featuredAlbum: Album, featuredSong: Song }) => {
  ...
  // Clases dinámicas
  const textColor = getRatingFont(current.rating);
  const borderStyle = getRatingBorder(current.rating);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-2xl">
            ...
            <p className="text-xl md:text-2xl text-zinc-400 mb-8 font-medium">
              por {
                current.artists.length === 0 ? 'Desconocido' : 
                current.artists.length === 1 ? current.artists[0].name : 
                current.artists.length === 2 ? `${current.artists[0].name} & ${current.artists[1].name}` :
                `${current.artists[0].name} and ${current.artists.length - 1} more`
              }
            </p>
