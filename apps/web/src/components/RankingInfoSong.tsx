...
const RankingInfoSong = ({ songData, cols = 4 }: { songData: Song, cols?: number }) => {
    const responsiveCols = gridColsClass[cols] ?? gridColsClass[4];

  return (
    <section className={`grid grid-cols-2 gap-3 ${responsiveCols}`}>
        {songData.rankings.map((ranking) => (
            <RankingCard key={ranking.id} title={ranking.title} rank={ranking.rank} subtitle={ranking.subtitle} />
        ))}
    </section>
  )
}
