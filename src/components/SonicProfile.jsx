    ...
    return (
        <article className="rounded-[2rem] bg-white/[0.02] p-6 md:p-8">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className={`flex items-center px-4 py-2 rounded-xl border ${getRatingBorder(data.rating)}`}>
            ...
            <div className={`grid grid-cols-1 gap-6 items-start ${image ? 'md:grid-cols-[220px_1fr]' : ''}`}>
                ...
                {/* GRID DE MÉTRICAS */}
                <div className={`grid grid-cols-2 gap-4 ${image ? 'sm:grid-cols-3' : 'sm:grid-cols-2 xl:grid-cols-3'}`}>
                    {metrics.map((stat) => (
                        <div
                            key={stat.label}
