...
export const searchAlbums = async (query) => {
    const response = await fetch(`http://localhost:3001/api/search/albums?q=${encodeURIComponent(query)}`);
    const albums = await response.json();
    return albums;
}

export const searchSongs = async (query) => {
    const response = await fetch(`http://localhost:3001/api/search/songs?q=${encodeURIComponent(query)}`);
    const songs = await response.json();
    return songs;
}

export const searchArtists = async (query) => {
    const response = await fetch(`http://localhost:3001/api/search/artists?q=${encodeURIComponent(query)}`);
